...
import preprocessing_metastases
from preprocessing_metastases import apply_preprocessing_task1
...
def cat(X_train, X_test, y_train, y_test):
    ...
    csv_pred.to_csv('part1/predicitions.csv', index=False)
...
def mission_zero(train_path, test_path):
    X_real_test = pd.read_csv(test_path)
    X_real_test.insert(loc=0, column='Unnamed', value=0)
    # X_real_test = preprocessingEli.basicPreprocessing(X_real_test)
    X_real_test = preprocessing_metastases.apply_preprocessing_task1(X_real_test)
    X = pd.read_csv(train_path)
    # X = preprocessingEli.basicPreprocessing(X)
    X = preprocessing_metastases.apply_preprocessing_task1(X)
    ...
    y_test = y_test['אבחנה-Location of distal metastases'].apply(
        lambda x: labels_shape(literal_eval(x)))
    (cat(X_train, X_real_test, y_train, y_test))

def mission_one(train_path, test_path):
    X_real_test = pd.read_csv(test_path)
    X_real_test.insert(loc=0, column='Unnamed', value=0)
    X_real_test = preprocessing_tumor_size.basicPreprocessing1(X_real_test)

    X = pd.read_csv(train_path)

    # todo: ose new processing
    X = preprocessing_tumor_size.basicPreprocessing1(X)
    ...
    # y_test = y_test['אבחנה-Location of distal metastases'].apply(
    #     lambda x: labels_shape(literal_eval(x)))
    cat_regressor(X_train, X_real_test, y_train, y_test)
...
def main():
    ...
    # test_path = args[1]
    # test_1_path = args[2]
    mission_zero('train.feats.csv', 'test.feats.csv')
    mission_one('train.feats.csv', 'test.feats.csv')
